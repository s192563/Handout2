...
## sheep and wolves
reproducesheep <- function(turtles, reproTurtles) {
  ...
  whoEnergy <- of(agents = reproInd, var = c("who", "energy"))
  who0 <- whoEnergy[which(whoEnergy[, "energy"] < energyreprosheep), "who"]
  if (length(who0) != 0) {
    reproInd <- die(turtles = reproInd, who = who0)
  }
...
reproducewolf <- function(turtles, reproTurtles) {
  # Throw dice to see if the turtles will reproduce
  repro <- runif(n = NLcount(turtles), min = 0, max = 100) < reproTurtles
  whoTurtles <- of(agents = turtles, var = "who") # "who" of the turtles before they reproduce
  reproWho <- whoTurtles[repro] # "who" of turtles which reproduce
  reproInd <- turtle(turtles, who = reproWho) # turtles which reproduce
  whoEnergy <- of(agents = reproInd, var = c("who", "energy"))
  who0 <- whoEnergy[which(whoEnergy[, "energy"] < energyreprowolf), "who"]
  if (length(who0) != 0) {
    reproInd <- die(turtles = reproInd, who = who0)
  }
...
while ((NLany(sheep) | NLany(wolves)) & time < maxTime) {
  ...
  if (NLcount(sheep) != 0) {
    ...
    sheep <- death(sheep)
    if (NLcount(sheep) != 0) {
      sheep <- reproducesheep(sheep, reproSheep)
    }
  }
  ...
  if (NLcount(wolves) != 0) {
    ...
    wolves <- death(wolves)
    if (NLcount(wolves) != 0) {
      wolves <- reproducewolf(wolves, reproWolf)
    }
  }
